...
// Copyright 2012-2023 by Satoru Takagi @ KDDI All Rights Reserved
//
// 2012/04/26 1st Ver
...
public class Shape2ImageSVGMap4 {
	...
	boolean inputJson = false; // for geoJsonSupport
	...
	public static void showHelp(){
		...
		System.out.println( "Copyright 2012 - 2023 by Satoru Takagi @ KDDI All Rights Reserved");
		System.out.println( "" );
		System.out.println( "Shape2ImageSVGMap TileSize (-Options) (Path4Shape.(shp|csv|json) (#fillColor | attrNumb) (#strokeColor | -) strokeWidth (symbolSize | sizedMarkerShapeName) )* " );
		System.out.println( "" );
...
//	    FeatureCollection<SimpleFeatureType, SimpleFeature> sfc = null;
...
//	  	System.out.println("FeatureCol. Bounds:" + sfc.getBounds() );
		...
		if ( attrColorNumber < 0 ){
			style = getFeatureStyle( fillColor , strokeColor , strokeWidth , dash , markerSize , sfSchema, markerName);
		} else {
...
//			style = getFeatureStyle2( attrColorNumber , strokeColor , strokeWidth ,  dash , markerSize , fs , fMin , fMax , markerName);
			style = getFeatureStyle2( attrColorNumber , strokeColor , strokeWidth ,  dash , markerSize , sfc , sfSchema , fMin , fMax , markerName); // 2019.6.7 インターフェースを変更(kmlやjsonに対応するため)
		}
...
//			targetCRS = CRS.decode("EPSG:900913",true);
			// 2023/05/17 上のdecodeが動かなくなった？ので、WKTから生成する方法を使う
			// from https://github.com/geoserver/geoserver/blob/main/data/release/user_projections/epsg.properties#L31
			String MERCATOR_WKT = "PROJCS[\"Google Mercator\",   GEOGCS[\"WGS 84\",     DATUM[\"World Geodetic System 1984\",       SPHEROID[\"WGS 84\", 6378137.0, 298.257223563, AUTHORITY[\"EPSG\",\"7030\"]],       AUTHORITY[\"EPSG\",\"6326\"]],     PRIMEM[\"Greenwich\", 0.0, AUTHORITY[\"EPSG\",\"8901\"]],     UNIT[\"degree\", 0.017453292519943295],     AXIS[\"Geodetic latitude\", NORTH],     AXIS[\"Geodetic longitude\", EAST],     AUTHORITY[\"EPSG\",\"4326\"]],   PROJECTION[\"Mercator (1SP)\", AUTHORITY[\"EPSG\",\"9804\"]],   PARAMETER[\"semi_major\", 6378137.0],   PARAMETER[\"semi_minor\", 6378137.0],   PARAMETER[\"latitude_of_origin\", 0.0],   PARAMETER[\"central_meridian\", 0.0],   PARAMETER[\"scale_factor\", 1.0],   PARAMETER[\"false_easting\", 0.0],   PARAMETER[\"false_northing\", 0.0],   UNIT[\"m\", 1.0],   AXIS[\"Easting\", EAST],   AXIS[\"Northing\", NORTH],   AUTHORITY[\"EPSG\",\"900913\"]]";
			targetCRS = CRS.parseWKT(MERCATOR_WKT);
		} else {
			targetCRS = crs;
		}
...
**/
	...
	public Style getFeatureStyle2( int colorCol , Color strokeColor , int strokeWidth , boolean dash , int markerSize ,  FeatureCollection<SimpleFeatureType, SimpleFeature> fsShape, SimpleFeatureType readFT, double fMin , double fMax , String markerName) throws Exception { // 2023/5/9 geojson supportのためにI/F変更
		boolean mixedType = isMixedType(readFT);
		System.out.println("Call getFeatureStyle2 : colorCol:" + colorCol + "  mixedType:" + mixedType );
		FeatureType schema = readFT;
		//スタイルを作成
		FilterFactory ff = CommonFactoryFinder.getFilterFactory2(null);
		StyleFactory sf = CommonFactoryFinder.getStyleFactory(null);
...
//			} else {
				colu = new SVGMapGetColorUtil( fsShape , colorCol , -1 , colorTable , keyLength , true , "" );
//			}
			colu.getAttrExtent(true);
		} else { // add 2016.2.4 (-colorkey)
			colu = new SVGMapGetColorUtil( fsShape , colorCol , -1 , SVGMapGetColorUtil.HSV , 128 , true , colorKeys );
		}
