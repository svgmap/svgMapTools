...
// Copyright 2007 - 2023 by Satoru Takagi
// 
// geoTools2.7.5 で動作確認
//
...
// 2023.05.09 geoJSON対応, geotools9.5に移行
// 
...
//	boolean noMetaId = false; // IDが入ると破綻するらしい？ 2017.4.17 svgmaptilestmでマルチスレッド化してIDがスレッド間重複を起こす・・
	...
	private static void showHelp(){
		...
		System.out.println("Copyright 2007-2023 by Satoru Takagi @ KDDI All Rights Reserved.");
		System.out.println("----------");
		System.out.println("java Shape2SVGMap [Options] (input.shp|input.csv) [output.svg]");
		System.out.println("input.(shp|csv|json) : ソースファイル指定。csvに関しては-csvschena説明参照。json:geoJson");
...
//				System.out.println("InputFile:" + infile + "  OutputFile:" + outfile);
			...
			if ( infile.endsWith(".csv")){
				...
				System.out.println("Shapefile or geojson input");
			}
...
//		gconv.tiny = true; // WGS<>Bessel変換を簡易版に
		...
		**/
		...
		//shp , csv ,(kml,json)ファイルなどから、FeatureCollectionとextentを取得する
		// 2019.6.7 関数に移行
		getFct fct = getFeatureCollectionAndExtent(  infile );
		FeatureCollection<SimpleFeatureType, SimpleFeature> fsShape = fct.fsShape;
		Envelope env = fct.env;
		SimpleFeatureType readFT = fct.readFT;
...
//			System.out.println("================olColorCol:"+olColorCol);
		...
		colorUtil = new SVGMapGetColorUtil(fsShape, colorCol, olColorCol, colorTable, colorKeyLength, strIsSJIS, strIsNative, colorKeys);
...
//				}
	...
	//shp , csv ,(kml,json)などから、FeatureCollectionとextentを取得する
	// 読み込み部分を関数に切り離した 2019/6/7
	private class getFct{
		FeatureCollection<SimpleFeatureType, SimpleFeature> fsShape;
		Envelope env;
		SimpleFeatureType readFT;
	}
	
	private getFct getFeatureCollectionAndExtent( String infile )throws Exception {
		
		
		if (infile.toLowerCase().endsWith("json")){
			return getJsonCollection( infile );
		}
		...
		if ( inputCsv ){
			...
			System.out.println("load shape file");
			//ロードするＳｈａｐｅ形式ファイル
...
//				hlc.putPlaneString("<rect x=\"" + (svgBounds.getMinX() ) + "\" y=\"" + (svgBounds.getMinY() ) + "\" width=\"" + svgBounds.getWidth() + "\" height=\"" + svgBounds.getHeight() + "\" fill=\"none\" stroke=\"#A00040\" stroke-width=\"" + (hlcWidth / 1000.0 ) + "\" />\n");
	...
	boolean strIsNative = false; // 2019.6.7 文字コードの変な変換をしないでそのままでいいタイプを追加（そろそろ正常化）
